...
| 步骤 | 状态 | 说明 |
|------|------|------|
| 阶段1.1: 创建统一存储封装 | ✅ | 已创建 `src/store/index.js` |
| 阶段1.2: 合并工具文件 | ✅ | utils/constants.js 与 src/utils/constants.js 内容相同，无需合并 |
| 阶段1.3: 更新 utils/api.js 导入路径 | ✅ | 修改为 `../src/utils/constants` |
| 阶段1.3: 修改 src/api/index.js 使用统一 storage | ✅ | 替换 localStorage 为 storage |
| 阶段1.3: 复制 utils/api.js 到 src/utils/ | ✅ | 修改导入路径为 `./constants` |
| 阶段1.3: 删除 utils/ 目录 | ✅ | 工具文件已合并到 src/utils/ |
| 阶段2.1: 配置 pages.json | ✅ | 所有页面已正确注册 |
| 阶段5.1: 更新 vite.config.js base path 为 / | ✅ | H5构建base路径改为根路径 |
| 阶段5.2: 更新 manifest.json H5 配置 | ✅ | 添加 base 和 router 配置 |

### 待完成

| 步骤 | 状态 | 说明 |
|------|------|------|
| 阶段2.2: 移除 Vue Router | ⏳ | 删除 src/router/index.js 和更新 src/main.js |
| 阶段3: 页面迁移 | ⏳ | 将 H5 页面逻辑迁移到 pages/ |
| 阶段4: 组件迁移 | ⏳ | 将组件从 src/components/ 适配 uni-app |
| 阶段6.1: 删除冗余文件 | ⏳ | 删除 src/views/ 目录 |
| 阶段6.3: 更新 CLAUDE.md | ⏳ | 更新项目文档 |
