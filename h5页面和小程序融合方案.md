...
| 步骤 | 状态 | 说明 |
|------|------|------|
| 阶段1.1: 创建统一存储封装 | ✅ | `src/store/index.js` |
| 阶段1.3: 合并工具文件 | ✅ | `utils/api.js` → `src/utils/api.js`，删除 `utils/` |
| 阶段1.3: 修改 src/api/index.js | ✅ | 使用 `storage` 替代 `localStorage` |
| 阶段2.1: 配置 pages.json | ✅ | 所有页面已正确注册 |
| 阶段2.2: 更新 src/router/index.js | ✅ | 使用 `storage` 替代 `localStorage`，更新 base path 为 `/` |
| 阶段2.2: 更新 src/App.vue | ✅ | 移除 base path，更新图标路径为 `/static/` |
| 阶段5.1: 更新 vite.config.js | ✅ | base path 改为 `/`，添加 `publicDir: 'static'` |
| 阶段5.2: 更新 manifest.json | ✅ | 添加 H5 base 配置 |
| H5 构建验证 | ✅ | 服务器正常启动在 `http://localhost:3002/` |

### 待完成

| 步骤 | 说明 |
|------|------|
| 阶段3: 页面迁移 | 将 `src/views/` 页面逻辑迁移到 `pages/` |
| 阶段4: 组件迁移 | 将组件从 `src/components/` 适配 uni-app |
| 阶段6.1: 删除 src/views/ | 页面迁移后删除 |
